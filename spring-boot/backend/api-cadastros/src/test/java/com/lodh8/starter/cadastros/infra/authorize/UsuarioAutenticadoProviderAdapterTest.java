...
class UsuarioAutenticadoProviderAdapterTest {
    ...
    @Test
    void atualizarNome_shouldUpdateRepositoryAndEvictCache() {
        UUID usuarioId = UUID.randomUUID();
        OAuthUser oAuthUser = mock(OAuthUser.class);
        when(oAuthUser.userId()).thenReturn("user-id");
        when(cacheManager.getCache("authenticated-user-cache")).thenReturn(cache);

        adapter.atualizarNome(usuarioId, "New Name", oAuthUser);

        verify(usuarioSistemaRepository).updateNome(usuarioId, "New Name");
        verify(cache).evict("user-id");
    }
}
